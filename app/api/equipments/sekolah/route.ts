...
export async function GET() {
  try {
    const query = `SELECT * FROM equipment_sekolah ORDER BY created_at ASC;`; 
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    const result = await pool.query(
      `DELETE FROM equipment_sekolah WHERE id = $1 RETURNING *;`,
      [Number(id)]
    );
